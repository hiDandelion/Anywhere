...
//  Anywhere
//
//  Manages a single VLESS mux connection with multiple multiplexed sessions.
//  Handles write serialization, receive-loop demultiplexing, and idle timeout.
//
...
class MuxClient {
    ...
    private func drainWriteQueue() {
        guard !isWriting, !writeQueue.isEmpty, let conn = vlessConnection else { return }

        isWriting = true
        let (data, completion) = writeQueue.removeFirst()

        conn.sendRaw(data: data) { [weak self] (error: Error?) in
            guard let self else { return }
            self.lwipQueue.async { [weak self] in
                guard let self else { return }
    ...
    private func resetIdleTimer() {
        ...
        timer.setEventHandler { [weak self] in
            ...
            if self.sessions.isEmpty {
                logger.debug("[Mux] Idle timeout, closing client")
                self.closeAll()
            }
        }
        timer.resume()
