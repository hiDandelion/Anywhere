...
//  Anywhere
//
//  Standard TLS 1.3 handshake client.
//  Adapted from RealityClient — same TLS 1.3 mechanics, but with standard
//  random SessionId, X.509 certificate validation, and CertificateVerify.
//  Returns a TLSRecordConnection for application data (identical record layer).
//
...
class TLSClient {
    ...
    private func sendClientFinished(completion: @escaping (Error?) -> Void) {
        ...
        do {
            let encrypted = try TLSRecordCrypto.encryptHandshakeRecord(
            ...
            // TLS record for encrypted Finished
            var finishedRecord = Data()
            finishedRecord.append(0x17) // Application Data
            finishedRecord.append(0x03)
            finishedRecord.append(0x03) // TLS 1.2 version
            finishedRecord.append(UInt8((encrypted.count >> 8) & 0xFF))
            finishedRecord.append(UInt8(encrypted.count & 0xFF))
            finishedRecord.append(encrypted)
